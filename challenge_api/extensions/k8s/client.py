    ...
    def create(self, data:ChallengeInfo, namespace="challenge") -> int:
        ...
        try:
            for event in w.stream(self.custom_api.list_namespaced_custom_object,
                                ...
                                plural="challenges",
                                timeout_seconds=300):  # 5분 타임아웃 추가
                obj = event['object']
        ...
        try:
            # 상태 업데이트
            recent_status = userchallenge_status_repo.get_recent_status(userchallenge.idx)
            if recent_status:
                userchallenge_status_repo.update_status(recent_status.idx, 'Running')
                userchallenge_status_repo.update_port(recent_status.idx, endpoint)
            else:
                userchallenge_status_repo.create(userchallenge_idx=userchallenge.idx, port=endpoint, status='Running')
        except Exception as e:
